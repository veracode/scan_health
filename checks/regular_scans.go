...
func regularScans(r *report.Report) {
	...
	r.ReportIssue(fmt.Sprintf("There have not been recent scans of this application The applciation was last scanned on %s which was %s ago. It is not uncommon for new flaws to be reported over time because Veracode is continuously improving their products, and because new SCA vulnerabilities are reported every day, and this could impact the application.", r.LastAppActivity, utils.FormatDuration(time.Since(r.LastAppActivity))), report.IssueSeverityMedium)
