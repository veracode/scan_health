...
func nodeModules(r *report.Report) {
	...
	if len(foundFiles) == 1 {
		r.ReportIssue(fmt.Sprintf("A \"node_modules\" folder was uploaded: \"%s\" %s", foundFiles[0], issueDescription), severity)
	} else {
		r.ReportIssue(fmt.Sprintf(
			"%d \"node_modules\" folders were uploaded: %s %s.",
			len(foundFiles),
			utils.Top5StringList(foundFiles), issueDescription), severity)
	}
