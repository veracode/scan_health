...
func moduleCount(r *report.Report) {
	if len(r.GetSelectedModules()) > utils.MaximumModuleSelectedCountThreshold {
		r.ReportIssue(fmt.Sprintf("%d modules were selected as entry points for analysis. This is a lot of modules and is an indicator that the module selection configuration may be suboptimal.", len(r.GetSelectedModules())), report.IssueSeverityMedium)
	...
	r.ReportIssue(fmt.Sprintf("%d modules were identified from the upload. This is a lot of modules and is an indicator that the compilation/upload may be suboptimal.", len(r.Modules)), report.IssueSeverityMedium)
	r.MakeRecommendation("Ensure the correct modules have been selected for analysis and that the packaging guidance has been followed.")
}
