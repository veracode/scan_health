...
func moduleCount(r *report.Report) {
	if len(r.GetSelectedModules()) > utils.MaximumModuleSelectedCountThreshold {
		...
		r.ReportModuleIssue(fmt.Sprintf("%d modules were selected as entry points for analysis. This is a lot of modules and is an indicator that the module selection configuration may be suboptimal.", len(r.GetSelectedModules())), report.IssueSeverityMedium, selectedModules)
	...
	for _, module := range r.Modules {
		if !utils.IsStringInStringArray(module.Name, modules) {
