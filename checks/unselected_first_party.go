...
func unselectedFirstParty(r *report.Report) {
	...
	message := fmt.Sprintf("A potential first-party module was not selected for analysis: \"%s\". Consider selecting this module for analysis if it is a first-party component and not a dependency/library.", foundModules[0])

	if len(foundModules) > 1 {
		message = fmt.Sprintf("%d potential first-party modules were not selected for analysis: %s. Consider selecting these modules for analysis if they are first-party components and not dependencies/libraries.", len(foundModules), utils.Top5StringList(foundModules))
	}
